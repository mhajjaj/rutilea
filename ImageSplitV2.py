...
image_dir = "C:\\Users\\mhajj\\Documents\\RUTILEA\\mhajjaj\\GearInspection-Dataset3\\CategoryNG\\ClassAll\\train\\"
...
def image_split_save(img, mask_img, y_split, x_left, x_right):
    img = img[:, x_left-10:x_right + 11]
    ...
    for i in range(len(y_split)-1):
        ...
        if np.mean(crop_mask_img) > 0:
            filtered_image = cv2.medianBlur(crop_img, ksize=3)
            clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
            filtered_image = cv2.cvtColor(filtered_image, cv2.COLOR_BGR2GRAY) 
            clahe_image_2 = clahe.apply(filtered_image)

            print(f"{image_dir}images\\{i:03}.png", str(clahe_image_2))
            # exit()
            imageio.imwrite(f"{image_dir}image_split\\{i:03}.png", clahe_image_2)
            imageio.imwrite(f"{image_dir}images_cleaned\\{i:03}_mask.png", crop_mask_img)
...
def main(input_directory):
    ...
    for img_path in load_image_files(input_directory_path / "images"):
        ...
        # last_element = input_directory.split('/')[-1]
        ...
        n = image_split_save(img, mask_img, y_split, x_left, x_right)

directories = glob.glob(f"{image_dir}")
